...
--
-- Host: 127.0.0.1
-- Generation Time: May 06, 2023 at 03:50 PM
...
(11, '2023-05-06 20:18:47', 'UPDATE LESSON: The 50 Most Popular Linux & Terminal Commands - Full Course for Beginners', 3, 4),
(12, '2023-05-06 20:26:41', 'UPDATE LESSON: Figma tutorial for Beginners: Complete Website from Start to Finish', 1, 5);
...
INSERT INTO `admin` (`id`, `username`, `password`, `email`, `first_name`, `last_name`, `gender`, `phone_number`, `photo_profile`, `status`) VALUES
(1, 'ivan', 'ivan123', 'ivan@gmail.com', 'Mochammad', 'Ivan Ra\'is', 'Male', '09876532121', 'ivan.png', 'Active'),
(3, 'joni', 'joni123', 'joni@gmail.com', 'Joni', 'Iskandar', 'Male', '23456789', 'joni.png', 'Active');
...
INSERT INTO `notification` (`id`, `date`, `lesson_suggestion_id`, `lesson_id`) VALUES
(5, '2023-05-06 20:09:34', 2, 4),
(6, '2023-05-06 20:24:22', 4, 5);
...
INSERT INTO `student` (`id`, `username`, `password`, `email`, `first_name`, `last_name`, `gender`, `phone_number`, `photo_profile`, `status`) VALUES
(1, 'ronaldo', 'ronaldo123', 'ronaldo@gmail.com', 'Cristiano', 'Ronaldo', 'Male', '456789', 'ronaldo.png', 'Active'),
(2, 'messi', 'messi123', 'messi@gmail.com', 'Lionel', 'Messi', 'Male', '3246214', 'messi.png', 'Active');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `notification_list`  AS SELECT concat('Your request has been approved: ',`lesson_suggestion`.`topic`) AS `message`, `notification`.`date` AS `date`, `lesson`.`link` AS `link`, concat(`admin`.`first_name`,' ',`admin`.`last_name`) AS `responded_by` FROM ((((`notification` join `lesson_suggestion` on(`notification`.`lesson_suggestion_id` = `lesson_suggestion`.`id`)) join `lesson` on(`notification`.`lesson_id` = `lesson`.`id`)) join `activity_log` on(`activity_log`.`lesson_id` = `lesson`.`id`)) join `admin` on(`activity_log`.`admin_id` = `admin`.`id`)) WHERE `activity_log`.`activity` like 'INSERT LESSON%''INSERT LESSON%'  ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `profile_admin`  AS SELECT `admin`.`first_name` AS `first_name`, `admin`.`last_name` AS `last_name`, `admin`.`email` AS `email`, `admin`.`gender` AS `gender`, `admin`.`phone_number` AS `phone_number`, `admin`.`photo_profile` AS `photo_profile` FROM `admin`;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `profile_student`  AS SELECT `student`.`first_name` AS `first_name`, `student`.`last_name` AS `last_name`, `student`.`email` AS `email`, `student`.`gender` AS `gender`, `student`.`phone_number` AS `phone_number`, `student`.`photo_profile` AS `photo_profile` FROM `student`;
...
ALTER TABLE `notification`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `UC_LessonSuggestionId` (`lesson_suggestion_id`),
  ADD UNIQUE KEY `UC_LessonId` (`lesson_id`),
