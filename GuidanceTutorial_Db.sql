...
CREATE TABLE notification(
    ...
    lesson_id INT NOT NULL,
    CONSTRAINT UC_LessonSuggestionId_LessonId UNIQUE(lesson_id, lesson_suggestion_id), 
    CONSTRAINT FK_Notification_LessonSuggestion FOREIGN KEY(lesson_suggestion_id) REFERENCES lesson_suggestion(id), 
    CONSTRAINT FK_Notification_Admin FOREIGN KEY(admin_id) REFERENCES admin(id), 
...
SELECT first_name, last_name, email, gender, phone_number FROM admin;

CREATE VIEW profile_student AS
SELECT first_name, last_name, email, gender, phone_number FROM student;
...
SELECT CONCAT('Your request has been approved: ', topic) AS message, notification.date AS date, link AS link, CONCAT(first_name, ' ', last_name) AS responded_by FROM lesson_suggestion
INNER JOIN notification ON lesson_suggestion.id = notification.lesson_suggestion_id
INNER JOIN lesson ON notification.lesson_id = lesson.id
INNER JOIN admin ON notification.admin_id = admin.id

SELECT * FROM notification_list WHERE responded_by = "Mochammad Ivan Ra'is";
-------------------- 
